...
def CreateSupport():
    ...
    s.unsetPrimaryObject()
    del mdb.models['Model-1'].sketches['__profile__']
def AddMalfunction(length , depth):
    s = mdb.models['Model-1'].ConstrainedSketch(name='__profile__', 
        sheetSize=200.0)
    g, v, c = s.geometry, s.vertices, s.constraints
    s.setPrimaryObject(option=STANDALONE)
    s.Line(point1=(length/2, length/2), point2=(-length/2, length/2))
    s.HorizontalConstraint(entity=g[2], addUndoState=False)
    s.Line(point1=(-length/2, length/2), point2=(-length/2, -length/2))
    s.VerticalConstraint(entity=g[3], addUndoState=False)
    s.PerpendicularConstraint(entity1=g[2], entity2=g[3], addUndoState=False)
    ...
    a.translate(instanceList=('Part-4-1', ), vector=(0.0, 0.0, D/2 - depth/2))
    #boolcut
    a.InstanceFromBooleanCut(name='Part-5', 
...
def GetMash(Shell_Size , Ball_Size , Support_Size):
    ...
    f = p.faces
    p.PartitionCellByExtendFace(extendFace=f[3], cells=c)
    p.PartitionCellByExtendFace(extendFace=f[15], cells=c)
...
def StepSetting(TimePeriod , numIntervals , Speed):
    ...
    a = mdb.models['Model-1'].rootAssembly
    s1 = a.instances['Part-5-1'].faces
    side1Faces1 = s1.findAt(((-D/2,0.0,0.0),(0.0,0.0,-D/2),))
    ...
    a = mdb.models['Model-1'].rootAssembly
    s1 = a.instances['Part-5-1'].faces
    side1Faces1 = s1.findAt(((-d/2,0.0,0.0),),
                            (((d/2) / (2 ** 0.5) , 0.0 , (d/2) /(2 ** 0.5)),),
                            (((d/2) /(2 ** 0.5) , 0.0 , -(d/2)/ (2 ** 0.5)),))
...
def Run(JobName,number_of_cpu):
    mdb.Job(name=JobName, model='Model-1', description='', type=ANALYSIS, 
        atTime=None, waitMinutes=0, waitHours=0, queue=None, memory=90, 
        memoryUnits=PERCENTAGE, explicitPrecision=SINGLE, 
        nodalOutputPrecision=SINGLE, echoPrint=OFF, modelPrint=OFF, 
        contactPrint=OFF, historyPrint=OFF, userSubroutine='', scratch='', 
        resultsFormat=ODB, parallelizationMethodExplicit=DOMAIN, numDomains=number_of_cpu, 
        activateLoadBalancing=False, multiprocessingMode=DEFAULT, numCpus=number_of_cpu)
